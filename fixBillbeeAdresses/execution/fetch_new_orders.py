"""
Fetch new orders from Billbee using settings from fetchBillbeeDocs.

Order state and lookback settings are read from:
  ../fetchBillbeeDocs/config/morning_fetch.yaml  (sibling project)
...
MORNING_FETCH_CONFIG = Path(__file__).parent.parent.parent / "fetchBillbeeDocs" / "config" / "morning_fetch.yaml"
...
def load_fetch_config() -> dict:
    """
    Load order fetch settings from fetchBillbeeDocs/config/morning_fetch.yaml.
    Exits with an error if the file is not found.
    Returns a dict with keys: state_ids (list[int]), lookback_hours (int).
    """
    if not MORNING_FETCH_CONFIG.exists():
        print(
            f"\nERROR: Config file not found:\n"
            f"  {MORNING_FETCH_CONFIG}\n\n"
            f"This file is part of the fetchBillbeeDocs project.\n"
            f"Make sure that project is checked out at:\n"
            f"  {MORNING_FETCH_CONFIG.parent.parent}\n",
            file=sys.stderr,
        )
        sys.exit(1)

    with open(MORNING_FETCH_CONFIG, encoding="utf-8") as f:
...
def fetch_orders_since(client, since: str, state_ids: list[int] | None = None) -> list[dict]:
    ...
    print(f"[fetch] {total_api} orders from API, {len(orders)} eligible for address check")
    return orders
