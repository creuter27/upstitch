...
Fetches orders that are in the after_fix state (e.g. 16 = Im Fulfillment),
meaning they have already had their addresses fixed and package types set
by fixAddressesAndPackaging. Creates DHL labels for all of them via polling.

The polling loop re-fetches each order and waits for a "Verpackungstyp: ..."
tag (set by Billbee automation after the state change in main.py) before
creating the label.
...
Which orders to fetch is determined by order_state_transitions.after_fix
in config/settings.yaml (falls back to order_state_id in morning_fetch.yaml).
"""
...
# Enable ANSI colours in Windows console
...
def main() -> None:
    ...
    # Load settings (common config merged with platform-specific override)
    ...
    # Determine which state to fetch orders from.
    # fixAddressesAndPackaging moved orders to after_fix state (e.g. 16 = Im Fulfillment).
    # We fetch from that state so we pick up exactly the orders that need labels.
    # Falls back to morning_fetch.yaml state_ids if after_fix is not configured.
    _after_fix_state = int(_transitions_cfg.get("after_fix") or 0)
    fetch_cfg = load_fetch_config()
    label_state_ids = [_after_fix_state] if _after_fix_state else fetch_cfg["state_ids"]
    ...
    _clog(
        ...
        f"(states={label_state_ids or 'all'})[/]"
    )

    client = BillbeeClient()
    orders = fetch_orders_since(client, since, state_ids=label_state_ids)
