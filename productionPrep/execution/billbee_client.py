...
# Pull in the shared client from the sibling repo by absolute path to avoid
# a circular import when this file is itself named billbee_client.py and is
# already registered in sys.modules under that name.
_base_path = Path(__file__).parent.parent.parent / "billbee-python-client" / "billbee_client.py"
_spec = importlib.util.spec_from_file_location("_billbee_client_base", _base_path)
_base_mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_base_mod)
_BaseBillbeeClient = _base_mod.BillbeeClient
